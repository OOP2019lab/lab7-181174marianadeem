...
bankAccount** readfile(string s);
void printAccounts(bankAccount**,int);
bool makeTransaction(bankAccount**b,int id,float balance,float*transactions,int num_of_transactions,float amount);
void deleteacc(bankAccount**,int size);
...

{
	...
	*/
	...
	b1->print();
	bool d1=b1->deposit(1000.0);
	bool d2=b1->deposit(3000.0);
	bool w1=b1->withdraw(10000.0);
	bool w2=b1->withdraw(6000.0);
	b1->print();
	...
	*/
	...
	const int n= 2;
	bankAccount *ptr[n];
	float *transactions1 =new float[3];
	transactions1[0]=3000.0;
	transactions1[1]=5000.0;
	transactions1[2]=60000.0;
	ptr[0]=new bankAccount(30,2000,transactions1,3);
	ptr[1]=new bankAccount(40,255000,400000);
	...
	*/
	int size=2;
	bankAccount **dyarray=new bankAccount*[size];
	float *transactions2 =new float[2];
	transactions2[0]=-5000;
	transactions2[1]=5000;
	dyarray[0]=new bankAccount(50,2000,transactions2,2);
	dyarray[1]=new bankAccount(60,6000,400000.0);
	bool w3=dyarray[0]->withdraw(1000.0);
	dyarray[0]->print();
	dyarray[1]->print();
...
bankAccount** readfile(string s)
{
	...
	fin.open("data.txt");
	{
		...
		else
		{
			...
			bankAccount **dyarray=new bankAccount*[accnum];
			for(int i=0;i<accnum;i++)
			{
				fin>>Id;
				fin>>balance;
			//	fin>>balance_limit;
				fin>>number_of_transactions;
				float *transactions =new float[number_of_transactions];
				for(int j=0;j<number_of_transactions;j++)
				{
					fin>>transactions[i];
				}
				dyarray[i]=new bankAccount(Id,balance,transactions,number_of_transactions);
				return dyarray;
			}
		}
	}
...
//bool makeTransaction(....){}
bool makeTransaction(bankAccount**b,int size,int id,float Balance,float*Transactions,int num_of_transactions,float amount)
{
	for(int i=0;i<size;i++)
	{for(int j=0;j<size;j++)
	{
		int tempid=b[i][j].getID();
		if(tempid==id)
		{
			int transactioncount=b[i][j].getTransLimit();
			if(transactioncount==num_of_transactions)
			{
				b[i][j].settransaction(amount);
				return true;
			}
			else
				cout<<"errror";
...
//void deleteAccounts(....){}
void deleteacc(bankAccount** b,int size)
{
	for(int i=0;i<size;i++)
	{
		delete b[i];
